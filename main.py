...
# Import custom logic from your separate engine files
...
# --- 2. STOCK UNIVERSE ---
# PASTE YOUR FULL STOCK_INDEX_MAPPING DICTIONARY HERE
STOCK_INDEX_MAPPING = {
    'MRF': 'NIFTY 500',
    'PAGEIND': 'NIFTY 500',
    'BOSCHLTD': 'NIFTY 500',
...
RAM_STATE = {
    ...
    "config": {
        side: {
            ...
            "risk_per_trade": 2000, "stop_loss_pct": 0.5,
            "risk_reward": "1:2", "trailing_sl": "1:1.5",
            "total_trades": 5, "trades_per_stock": 2
        } for side in ["bull", "bear", "mom_bull", "mom_bear"]
    },
    "engine_live": {"bull": False, "bear": False, "mom_bull": False, "mom_bear": False},
    "pnl": {"total": 0.0, "bull": 0.0, "bear": 0.0, "mom_bull": 0.0, "mom_bear": 0.0},
    "data_connected": {"breakout": False, "momentum": False},
    "manual_exits": set(),
    "banned": set(),
    "sentiment": {"pct": 0, "bull": 0, "bear": 0}
}


# --- 4. KITE TICKER HANDLERS ---
...
async def kite_login_redirect():
    if not RAM_STATE["api_key"]: raise HTTPException(400, "Save API Key First")
    return RedirectResponse(url=KiteConnect(api_key=RAM_STATE["api_key"]).login_url())


@app.get("/login")
async def kite_callback(request_token: str = None):
    try:
        kite = KiteConnect(api_key=RAM_STATE["api_key"])
        ...
        for instr in instruments:
            ...
            if symbol in STOCK_INDEX_MAPPING:
                ...
                stock_obj = {
                    'symbol': symbol, 'ltp': 0, 'status': 'WAITING',
                    'hi': 0, 'lo': 0, 'pdh': 0, 'pdl': 0, 'sma': 0, 'trades': 0,
                    'candle': None, 'last_vol': 0, 'side_latch': None
                }
                RAM_STATE["stocks"][token] = stock_obj
                RAM_STATE["stocks_by_symbol"][symbol] = stock_obj
        ...
        return HTMLResponse("<h2>Authenticated!</h2><script>setTimeout(() => window.close(), 1000)</script>")
...
async def get_stats():
    for side in ["bull", "bear", "mom_bull", "mom_bear"]:
        total = 0.0
        for t in RAM_STATE["trades"][side]:
            t['pnl'] = (t['ltp'] - t['entry_price'] if t['side'] == 'BUY' else t['entry_price'] - t['ltp']) * t['qty']
            total += t['pnl']
        RAM_STATE["pnl"][side] = total
    RAM_STATE["pnl"]["total"] = sum(RAM_STATE["pnl"][s] for s in ["bull", "bear", "mom_bull", "mom_bear"])
    return {
        "data_connected": RAM_STATE["data_connected"],
        "sentiment": {"pct": 65, "bull": 40, "bear": 20},
        "pnl": RAM_STATE["pnl"],
...
async def get_engine_params(side: str): return RAM_STATE["config"].get(side, {})
