...
# It prevents the "ValueError: signal only works in main thread" error on Heroku.
...
# --- STOCK UNIVERSE ---
# Populated with a sample; ensure your production version contains all 500+ tokens
STOCK_INDEX_MAPPING = {
    'MRF': 'NIFTY 500',
    '3MINDIA': 'NIFTY 500',
    'HONAUT': 'NIFTY 500',
...
RAM_STATE = {
    "main_loop": None, # Stores the reference to the FastAPI uvloop
    "kite": None,
    "kws": None,
    "api_key": "",
    "api_secret": "",
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if token in RAM_STATE["stocks"]:
            ...
            # 1. Update LTP in RAM immediately
...
async def startup_event():
    ...
    api_secret = sec_r = sec_redis or os.getenv("KITE_API_SECRET", "")
    ...
    if token and RAM_STATE["api_key"]:
        try:
            ...
            logger.info("🛰️ SYSTEM: Ticker initialized in background thread.")
...
async def save_engine_settings(side: str, data: dict):
    if side in RAM_STATE["config"]:
        ...
        logger.info(f"📝 SETTINGS: Config updated for {side}.")
...
async def kite_callback(request_token: str = None):
    ...
    try:
        # Resolve Credentials (RAM -> ENV -> Redis)
        api_key = RAM_STATE["api_key"] or os.getenv("KITE_API_KEY")
        ...
        if not api_key or not api_secret:
            raise ValueError("API Key or Secret missing during callback.")
...
if __name__ == "__main__":
    ...
    # CRITICAL: Use workers=1 to maintain single-process RAM state consistency
